...
export const ChatView = ({
  ...
  return (
    <div className="flex-1 flex flex-col h-full w-full bg-[var(--bg-panel)] transition-colors duration-200">
      ...
      <div className="px-3 py-3 sm:p-4 bg-[var(--bg-header)] border-b border-[var(--border-color)] flex items-center justify-between gap-2 transition-colors duration-200">
      ...
      <div className="flex items-center gap-1.5 sm:gap-3 shrink-0">
        <button
          ...
          className={`px-2 sm:px-3 py-1.5 border rounded text-[10px] sm:text-xs font-medium transition active:scale-95 flex items-center gap-1 ${
            preserveHistory
        ...
        <button
          ...
          className="px-2 sm:px-3 py-1.5 bg-[var(--bg-sidebar)] hover:bg-[var(--bg-main)] border border-[var(--border-color)] rounded text-[10px] sm:text-xs font-medium text-[var(--text-muted)] hover:text-[var(--text-main)] transition active:scale-95 shrink-0 flex items-center gap-1"
        >
          <span>📹</span>
      ...
      <div className="flex-1 overflow-y-auto p-3 sm:p-4 space-y-3 bg-[var(--bg-panel)] transition-colors duration-200">
        {messages.length === 0 ? (
      ...
      <div className="p-3 sm:p-4 bg-[var(--bg-header)] border-t border-[var(--border-color)] transition-colors duration-200">
        <form onSubmit={handleSubmit} className="flex space-x-2 max-w-5xl mx-auto">
          ...
          <button
            type="submit"
            className="px-4 sm:px-6 py-2.5 bg-blue-600 hover:bg-blue-700 font-semibold tracking-wide rounded-lg text-sm text-white transition active:scale-95 shrink-0"
          >
            send
          </button>
        </form>
